...
%	),
	...
	(   ( memberchk(grant(true),Opts), memberchk(deny(true),Opts) )
...
%setpol1(all) :- !, setpol2(all).
%setpol1(allc) :- !, spld:clear_policy, setpol2(all).
%
%setpol2(P) :- spld:policy(P,_), !,
% param:setparam(current_policy,P),writeln(success).
